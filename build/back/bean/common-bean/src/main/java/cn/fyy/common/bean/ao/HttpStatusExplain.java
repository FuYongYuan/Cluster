...
public class HttpStatusExplain {
     ...
     * Security状态中文说明集合
     */
    private static final TreeMap<HttpStatus, String> SECURITY_STATUS = new TreeMap<>();

    /*
     * 初始化Security状态中文说明集合
     */
    static {
        SECURITY_STATUS.put(HttpStatus.UNAUTHORIZED, "用户未登录或登录信息已过期，请检查登录信息！");
        SECURITY_STATUS.put(HttpStatus.FORBIDDEN, "当前接口您没有权限访问，请检查用户权限！");
    }
     ...
     * @return 中文说明
     */
    public static String getSecurityStatus(HttpStatus httpStatus) {
        return SECURITY_STATUS.get(httpStatus);
    }
}
