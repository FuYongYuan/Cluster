...
import {LocalStorageAccountMenuList} from "@/apis/commons/constant";
    ...
    setup() {
        ...
        const changeTab = (id: number) => {
            ...
            if (contentData.activeKey === contentData.homeId) {
                ...
                const routeItem = contentData.visitedRoutes.find((routeItem) => routeItem.id === id);
                if (routeItem) {
        ...
        watch(() => route.fullPath, (to) => {
            if (contentData.homeUrl !== to) {
                const routeItem = contentData.visitedRoutes.find((routeItem) => routeItem.fullPath === to);
                if (!routeItem) {
        ...
        const removeTab = (id: number) => {
            const visitedRoute = contentData.visitedRoutes.find((routeItem) => id === routeItem.id);
            if (visitedRoute) {
                contentData.visitedRoutes.forEach((routeItem, index) => {
                    if (routeItem.fullPath === visitedRoute.fullPath) {
                        ...
                        if (contentData.activeKey === visitedRoute.id) {
                            // 关闭最后一页-显示删除后的最后一页
                            if (contentData.visitedRoutes.length <= index && contentData.visitedRoutes.length > 0) {
                                changeTab(contentData.visitedRoutes[contentData.visitedRoutes.length - 1].id);
                            }
                            // 没有页签时候显示首页
        ...
        // 动态填写缓存
        const isKeepAlive = (keepAlive: boolean, visitedRoute: VisitedRouteType) => {
            if (keepAlive) {
                if (visitedRoute.pageName != undefined && visitedRoute.pageName !== "") {
                    if (!contentData.include.find((menuName) => menuName === visitedRoute.pageName)) {
                ...
                if (visitedRoute.pageName != undefined && visitedRoute.pageName !== "") {
                    if (contentData.include.find((menuName) => menuName === visitedRoute.pageName)) {
                        contentData.include.splice(contentData.include.indexOf(visitedRoute.pageName), 1);
                    }
                }
            }
        }
