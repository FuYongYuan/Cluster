...
import {getManagerByJwtToken, saveManager} from "@/apis/member/service";
import {FormInstance, message, UploadChangeParam} from "ant-design-vue";
import {ManagerDTO} from "DTO";
import {uploadFormData} from "@/apis/data/service";
import {imageFormatValidate, mailFormatValidate, phoneFormatValidate} from "@/utils/validate/regex";
import {RuleObject} from "ant-design-vue/es/form";
import {BusinessTypeUserHeadImage} from "@/apis/commons/constant";
    ...
    setup(props, context) {
        ...
        const handleOk = async () => {
            detail.value?.validate().then(async () => {
                ...
                // 保存
                const result = await saveManager(
                    detailData.form
                );
                // 保存结果
