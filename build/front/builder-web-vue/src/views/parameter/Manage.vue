<template>
    <div>
        ...
        >
                ...
                <a-col :span="12" class="div-right">
                    <a-form-item>
                        <a-space>
                            ...
                            <a-button class="ant-btn-daybreak" :loading="loadingState" @click="exportList">
                                <icon-font type="icon-export" v-if="!loadingState"/>
                                导出
                            </a-button>
                        </a-space>
                    </a-form-item>
            ...
            <a-row :gutter="16">
                <a-col :span="6">
                    <a-form-item label="参数编码" name="parameterCode">
                        <a-input v-model:value="searchFormData.parameterCode" placeholder="参数编码" allowClear/>
                    </a-form-item>
                </a-col>
                <a-col :span="6">
                    <a-form-item label="参数名称" name="parameterName">
                        <a-input v-model:value="searchFormData.parameterName" placeholder="参数名称" allowClear/>
                    </a-form-item>
                </a-col>
                <a-col :span="6">
        ...
        <div class="search-result">
            ...
            >
                <template #bodyCell="{ column,record }">
                    <!--          <template v-if="column.dataIndex === 'parameterExplain'">-->
                    <!--            <a-tooltip placement="topLeft" :title="record.parameterExplain">-->
                    <!--              {{ record.parameterExplain }}-->
                    <!--            </a-tooltip>-->
                    <!--          </template>-->
...
import {FormInstance, message, Table, TablePaginationConfig} from 'ant-design-vue';
import {deleteParameter, exportParameter, queryParameter} from "@/apis/dictionary/service";
import {getPageSort, getSerializableSort} from "@/apis/commons/method";
import ParameterDetail from "@/views/parameter/Detail.vue";
...
export default defineComponent({
    ...
    setup() {
        ...
        const pagination = reactive<TablePaginationConfig>({
            ...
            size: "default",
            // 显示总数等
            showTotal: (total: number, range: any) => `第 ${range[0]} 至 ${range[1]} 条，共计 ${total} 条`,
        });
        ...
        const loading = async () => {
            ...
            // 调用查询
            const result = await queryParameter(
                ...
                manageData.searchFormData.parameterCode,
                manageData.searchFormData.parameterName,
        ...
        const deleteAll = async () => {
            if (manageData.selectedRowKeys.length > 0) {
                ...
                manageData.loadingState = true;
                // 删除
                const result = await deleteParameter(manageData.selectedRowKeys.join(","));
        ...
        const handleShowDetail = (id: number) => {
            manageData.parameterDetailVisible = true;
            parameterDetail.value.getById(id);
        }
