...
1. 前端页面多个重复菜单，需要看看怎么回事，一个点击有效一个无效，功能的url要修改成带上地址中的参数，但是要写个正则表达式能让他自动判断/后带什么都可以这样才能完成地址完整校验。
2. 还有按钮权限、数据权限、接口权限没有添加，也没有写后台校验各种权限的逻辑
3. 前后端权限校验没有接口的权限校验，只要登陆账号后，知道地址即可访问
4. 用户权限体系重构成符合 账户身上有职位，有角色，角色有权限，等多重权限体系
5. 页面上加入数据权限及按钮权限控制
